...
function handleNoteEvent(noteEvent: NoteEvent) {
  console.log(`🎵 ${noteEvent.type.toUpperCase()}: ${noteEvent.noteFullName} (vel: ${noteEvent.velocity})`);
...
export async function performanceTest() {
  ...
  console.log(`Captured ${eventCount} events (${(eventCount / duration * 1000).toFixed(2)} events/sec)`);
}
