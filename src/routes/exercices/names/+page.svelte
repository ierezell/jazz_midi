...
<script lang="ts">
	...
	// React to changes in the selected note from the dropdown
	$effect(() => {
	...
	function generateNewNote(): void {
		if (englishToLatin) {
			...
			// Use the selected note from the dropdown instead of random
			currentDisplayNote = currentTargetNote;
		} else {
			// Show Latin note, expect English equivalent
			// Use the selected note from the dropdown instead of random
			const latinNote = ENGLISH_TO_LATIN[currentTargetNote];
	...
	function generateExpectedNotes(selectedNote: Note): MidiNote[] {
		// For this exercise, we expect a single note to be played
		// Use the selectedNote from the dropdown
		currentTargetNote = selectedNote;
		const rootNoteName = (selectedNote + DEFAULT_OCTAVE) as NoteFullName;
		const rootMidi = NoteToMidi[rootNoteName];

		console.debug(
			`Note name exercise: ${currentDisplayNote} -> ${selectedNote}`,
	...
	function validateNoteName(
		...
		if (expectedNotes.includes(event.noteNumber)) {
			...
			const correctAnswer = englishToLatin ? ENGLISH_TO_LATIN[selectedNote] : selectedNote;
	...
	function handleNextNote(): void {
		// In non-random mode, just reset the current note exercise
		// The user can change the note using the dropdown
		playedCorrectly = false;
		exerciseCompleted = false;
		generateNewNote();
	}
