...
<script lang="ts">
	...
	function validateNoteEvent(_selectedNote: Note, event: NoteEvent): ValidationResult {
		const result = validateHandDynamics(
			event,
			{
				lhTargetMin: LH_TARGET.min,
				lhTargetMax: LH_TARGET.max,
				rhTargetMin: RH_TARGET.min,
				rhTargetMax: RH_TARGET.max,
				tolerance: 5,
				lhNotes: LH_SHELLS,
				rhNotes: RH_SCALE
			},
	...
	let lhAcc = $derived(
		state.lhNotesPlayed > 0 ? Math.round((state.lhCorrectDynamics / state.lhNotesPlayed) * 100) : 0
	);
	let rhAcc = $derived(
		state.rhNotesPlayed > 0 ? Math.round((state.rhCorrectDynamics / state.rhNotesPlayed) * 100) : 0
	);
</script>
