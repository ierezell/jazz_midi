<script lang="ts">
	...
	async function checkPermission() {
		...
		try {
			const result = await navigator.permissions.query({ name: 'midi' as PermissionName, sysex: false } as PermissionDescriptor);
			permissionState = result.state as typeof permissionState;
			result.onchange = () => { permissionState = result.state as typeof permissionState; };
		} catch {
	...
	>
		...
		>
			...
			<div class="troubleshoot-steps">
				...
				<ol>
					<li>Plug in your MIDI keyboard via USB, then click <strong>Grant MIDI Access</strong> above.</li>
					<li>If no popup appeared: click the <strong>🔒 lock icon</strong> in the address bar → <em>Site permissions</em> → set MIDI to <strong>Allow</strong>.</li>
