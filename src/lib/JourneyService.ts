...
export class JourneyService {
	...
	async loadFromSupabase(userId: string): Promise<void> {
		...
		const [unitResult, lessonResult] = await Promise.all([
			supabase.from('unit_progress').select('*').eq('user_id', userId),
			supabase.from('lesson_progress').select('*').eq('user_id', userId)
		]);
		const unitMap = new Map((unitResult.data ?? []).map(r => [r.unit_id, r.status]));
		const lessonMap = new Map((lessonResult.data ?? []).map(r => [r.lesson_id, r]));
		for (const unit of this.units) {
			const savedStatus = unitMap.get(unit.id);
			if (savedStatus) unit.status = savedStatus;
			for (const lesson of unit.lessons) {
				const savedLesson = lessonMap.get(lesson.id);
				if (savedLesson) {
					lesson.completed = savedLesson.completed;
					lesson.stars = savedLesson.stars;
					lesson.perfectCompletions = savedLesson.perfect_completions;
				}
			}
		}
	}

	private syncToSupabase(progress: object[]): void {
		if (!this.supabaseUserId) return;
		const userId = this.supabaseUserId;
		import('./supabaseClient').then(({ supabase }) => {
			const unitRows = (progress as { id: string; status: string }[]).map(u => ({
				user_id: userId, unit_id: u.id, status: u.status
			}));
			const lessonRows = (progress as { lessons: { id: string; completed: boolean; stars: number; perfectCompletions: number }[] }[])
				.flatMap(u => u.lessons.map(l => ({
					user_id: userId, lesson_id: l.id,
					completed: l.completed, stars: l.stars, perfect_completions: l.perfectCompletions
				})));
			Promise.all([
				supabase.from('unit_progress').upsert(unitRows),
				supabase.from('lesson_progress').upsert(lessonRows)
			]).then(results => {
				const err = results.find(r => r?.error);
				if (err?.error) console.warn('Supabase journey sync:', err.error.message);
			});
		});
	}
}
