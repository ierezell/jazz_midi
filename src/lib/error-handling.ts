 ...
 */
export function constrainToRange(note: number, min: MidiNote, max: MidiNote): MidiNote {
 ...
 */
export function safeArrayAccess<T>(array: ReadonlyArray<T>, index: number, fallback: T): T {
 ...
 */
export function safeCalculation<T>(calculation: () => T, fallback: T, context?: string): T {
	try {
		return calculation();
	} catch (error) {
