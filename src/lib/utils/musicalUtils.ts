...
export class ChordUtils {
    ...
    private static applyVoicing(chord: Chord, voicing: 'open' | 'drop2' | 'drop3'): Chord {
        const notes = [chord.root, chord.third, chord.fifth, chord.seventh].filter(n => n !== undefined) as MidiNote[];
...
export class ScaleUtils {
     ...
     */
    static getMode(root: Note, mode: 'ionian' | 'dorian' | 'phrygian' | 'lydian' | 'mixolydian' | 'aeolian' | 'locrian'): NoteFullName[] {
...
export class VoiceLeadingUtils {
     ...
     */
    static calculateVoiceLeading(chord1: Chord, chord2: Chord): {
