...
function analyzeProgression(
	...
	const isMajorQuality = (q: string) =>
		q === '' || q === 'maj' || q === 'maj7' || q === '6' || q === 'major';
...
export function analyzeChordTone(
	...
	const isChordTone = ['Root', 'Major 3rd', 'Minor 3rd', '5th', 'Minor 7th', 'Major 7th'].includes(
		role
	);
...
export function getVoicingSuggestion(
	...
	const chordNotes = [chordData.root, chordData.third, chordData.fifth, chordData.seventh].filter(
	...
	if (chordNotes.length >= 3) {
		...
		const rightHandNotes = [chordData.third, chordData.seventh || chordData.fifth]
...
export function generatePracticeHints(chords: SongChord[]): string[] {
	...
	if (progressions.length > 0) {
		hints.push(
			`Found ${progressions.length} II-V-I progression(s) - these are the building blocks of jazz harmony!`
		);
	}
