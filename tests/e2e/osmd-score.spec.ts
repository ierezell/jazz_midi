...
test.describe('OSMD Score Rendering', () => {
	test('should render MusicXML in song-chords exercise', async ({ page }) => {
		...
		await expect(page.locator('.osmd-container, .musicxml-score-container')).toBeVisible({ timeout: 15_000 });
	...
	test('should display loading state while score loads', async ({ page }) => {
		...
		await expect(page.locator('.loading-indicator, .loading-overlay')).toBeVisible({ timeout: 5_000 });
	...
	test('should handle zoom controls', async ({ page }) => {
		...
		// Get initial width
		const initialWidth = await page.locator('.osmd-container svg').evaluate(el => el.getBoundingClientRect().width);
		...
		if (await zoomInBtn.isVisible().catch(() => false)) {
			...
			// Should be larger
			const newWidth = await page.locator('.osmd-container svg').evaluate(el => el.getBoundingClientRect().width);
	...
	test('should be responsive on mobile', async ({ page }) => {
		...
		const svgWidth = await page.locator('.osmd-container svg').evaluate(el => el.getBoundingClientRect().width);
