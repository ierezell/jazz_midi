...
import { NoteToMidi, MidiToNote } from '../../src/lib/types/notes.constants';
import { chords } from '../../src/lib/MusicTheoryUtils';
...
describe('II-V-I progression helper logic', () => {
	it('C II-V-I progression generates expected notes for each chord', () => {
		...
		const expectedNames = progression.map((ch) =>
			[ch.root, ch.third, ch.fifth, ch.seventh]
				.filter(Boolean)
